...
const swal = Swal;
@Component({
  selector: 'app-proposal-published-owner',
  templateUrl: './proposal-published-owner.component.html',
  styleUrls: ['./proposal-published-owner.component.scss']
})
export class ProposalPublishedOwnerComponent implements OnInit {
        ...
        this.http.get(this.global.api+'api.php?action=proposallists&user='+this.global.userid,this.global.option)
          ...
          .subscribe(res => {
            ...
            for (var i = 0; i < res.length; ++i) {
              if (res[i].status[0].name=="Qualified"||res[i].status[0].name=="Approved"||res[i].status[0].name=="Evaluated"||res[i].status[0].name=="GrantRelease"||res[i].status[0].name=="Signed and Approved") {
